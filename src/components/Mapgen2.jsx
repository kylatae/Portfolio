...
function MapGrid() {
  ...
  function checkAndAdjustTile(grid, row, col, type) {
    ...
    const clusters = identifyClusters(grid, visited);
    const smallClusters = clusters.filter(c => c.size < threshold); // Adjust 'threshold' as needed
  
    // for (const cluster of smallClusters) {
    //    attemptToConnectCluster(grid, cluster); 
    // } 
  }
  ...
  function identifyClusters(grid, visited) {
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[0].length; j++) {
        if (grid[i][j] === 'water' && !visited.has(`${i}-${j}`)) {
          const cluster = [];
          findCluster(grid, visited, i, j, cluster);
  ...
  function findCluster(grid, visited, row, col, cluster) {
    const key = `${row}-${col}`;
    ...
    // Check neighbors similar to the flood fill
    const neighbors = [
      [row - 1, col], 
      [row + 1, col], 
      [row, col - 1], 
    ...
    for (const [newRow, newCol] of neighbors) {
      if (newRow >= 0 && newRow < grid.length &&
          newCol >= 0 && newCol < grid[0].length &&
          grid[newRow][newCol] === 'water') {
        findCluster(grid, visited, newRow, newCol, cluster);
      }
    }
  }
  ...
  function attemptToConnectCluster(grid, cluster) {
    // ... Logic to find the best connection point to another cluster
    // ... or a suitable path to connect tiles within the cluster.
    // ... Modify the grid accordingly by changing tiles to 'water'
  }
